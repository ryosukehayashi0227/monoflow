...
const CONSTANTS = {
    ...
    I18N: {
        ja: {
            ...
            help_m1_h1: '視覚的要素の詳細',
            help_m1_l1: '<strong>優先度アクセント:</strong> 左端のライン色は「高(赤)」「中(橙)」「低(青)」を表し、緊急度を直感的に伝えます。',
            help_m1_l2: '<strong>動的なタイムスタンプ:</strong> 作成日時に加え、内容を編集すると「更新日時」が自動記録されます。完了時は完了時刻も表示されます。',
            help_m1_l3: '<strong>メモプレビュー:</strong> 詳細メモの冒頭2行を表示。カードを広げずにコンテキストを確認でき、情報密度を維持します。',
            help_m1_l4: '<strong>期限バッジ:</strong> 今日を過ぎた期限は「赤」で強調され、見落としを防ぎます。',
            help_m1_h2: 'ステータス管理',
            help_m1_l5: 'Doneレーンに移動したタスクは、自動的に透明度が下がり「完了済み」として視覚的に区別されます。これは新しい未完了タスクへの集中を助けるための意図的なデザインです。',
            help_m1_l6: 'タイトルには打ち消し線が適用され、心理的な達成感を高めます。',
            help_m1_l7: '完了日時はISO形式で厳密に記録され、統計データ（Metrics）の正確なソースとなります。',
            help_m2_t: '2. 高度な親子関係と階層化',
            help_m2_d: 'MonoFlowは「Parent-Child」の2階層構造を厳格に維持することで、管理の複雑化を防ぎながら強力な整理機能を提供します。',
            help_m2_h1: '仮想親チケット (Context Ghost)',
            help_m2_l1: '子タスクが親と異なるカラムにある場合、親の「残像」が表示されます。これにより「この細かい作業は、元々どのプロジェクトのものか」という文脈を失わずに済みます。',
            help_m2_h2: '仮想子チケット (Subtask Preview)',
            help_m2_l2: '親タスクの下には、他カラムに散らばっている全子タスクの状況がコンパクトなリストとして集約されます。',
            help_m2_h3: 'ジャンプ＆フラッシュ',
            help_m2_l3: '仮想チケットをクリックすると、実体のある場所まで一瞬でスクロールし、青く「フラッシュ」して強調します。広大なボード内での迷子を完全に防ぎます。',
            help_m2_extra: '親タスクにはサブタスク全体の消化率（%）とプログレスバーが表示され、プロジェクトの全体進捗をリアルタイムに把握できます。',
            help_m2_l4: '<strong>階層の制限:</strong> 3層以上のネストは情報の迷路を生むため、MonoFlowでは意図的に「親と子」の2層までに制限しています。すでに子を持つタスクを別タスクの子にすることはできません。',
            help_m3_t: '3. 検索と動的フィルタリング',
            help_m3_d: '膨大なタスク群から、必要な情報だけを抽出するための複数のアプローチを用意しています。',
            help_m3_h1: 'リアルタイム全文検索',
            help_m3_l1: 'タイトルだけでなく「詳細メモ」の内容も検索対象です。入力と同時にボードがフィルタリングされます。',
            help_m3_h2: 'マルチレイヤー・フィルタ',
            help_m3_l2: '「特定のラベル」かつ「高優先度」のみを表示するなど、複数の条件を組み合わせた絞り込みが可能です。',
            help_m3_extra: '注：フィルタ適用中はデータの安全性を守るため、ドラッグ＆ドロップによる並べ替えが一時的に無効化されます。',
            help_m4_t: '4. 進捗分析とデータサイエンス',
            help_m4_d: 'MonoFlowはあなたの活動をログとして蓄積し、客観的な数値でパフォーマンスを可視化します。',
            help_m4_h1: 'Metrics (指標)',
            help_m4_l1: '<strong>平均完了日数 (Lead Time):</strong> タスク作成から完了までにかかった時間の平均。自分のキャパシティを把握する重要な指標です。',
            help_m4_h2: 'Burndown (燃焼図)',
            help_m4_l2: '理想線（点線）と実績線（青実線）を比較することで、計画に対する遅れを視覚的に把握できます。',
            help_m4_h3: 'アーカイブ仕様',
            help_m4_l3: 'アーカイブは削除ではありません。完了したタスクを視界から消しつつ、統計データには残しておくための「整理棚」です。復元時は元のカラムへ正確に戻ります。',
            help_m4_privacy_t: 'データプライバシー',
            help_m4_privacy_d: 'すべてのデータは、あなたのPCのブラウザ（LocalStorage）にのみ保存されます。MonoFlowのサーバーが存在しないため、あなたの機密情報が外部へ流出する可能性は物理的にゼロです。',
            help_m5_t: '5. カスタマイズと効率化',
            help_m5_h1: 'ダークモード',
            help_m5_l1: 'システム設定に連動するだけでなく、手動でいつでも切り替え可能。夜間の集中力をサポートします。',
            help_m5_h2: '完全なバックアップ',
            help_m5_l2: '「エクスポート」はタスクだけでなく、あなたの設定（言語・テーマ）もすべて含んだJSONを出力します。',
        ...
        en: {
            ...
            help_m1_d: 'MonoFlow task cards are designed to prioritize information and provide an immediate overview of your work.',
            ...
            help_m1_l3: '<strong>Note Preview:</strong> Shows the first two lines of your notes, providing context without cluttering the board.',
            help_m1_l4: '<strong>Status Badges:</strong> Overdue deadlines are highlighted in red to ensure they are never missed.',
            ...
            help_m1_l6: 'Titles are struck through to provide a clear psychological sense of accomplishment.',
            ...
            help_m2_d: 'MonoFlow uses a strict Parent-Child structure to keep your workflow simple yet capable of handling complex projects.',
            ...
            help_m3_d: 'Quickly find what you need, even when managing thousands of individual tasks.',
            ...
            help_m3_l2: 'Combine label selections and priority filters for powerful data slicing and focused work sessions.',
            help_m3_extra: 'Note: Drag & Drop is temporarily disabled while filters are active to prevent accidental data corruption.',
            help_m4_t: '4. Analytics & Productivity Science',
            help_m4_d: 'MonoFlow turns your task history into actionable data to help you evaluate and improve your performance.',
            ...
            help_m4_l2: 'Compare your "Actual Remaining Work" against the "Ideal Progress Line" to see if you are ahead or behind schedule.',
            help_m4_h3: 'Archive Logic',
            help_m4_l3: 'Archiving hides tasks from view but preserves their data for historical metrics. Restoring accurately returns them to their original lanes.',
            help_m4_privacy_t: 'Data Privacy',
            help_m4_privacy_d: 'All data stays in your browser\'s LocalStorage. With no central server, your data privacy is physically guaranteed—no leaks, no tracking.',
            help_m5_t: '5. Customization & Efficiency',
            help_m5_h1: 'Dark Mode',
            help_m5_l1: 'Integrated support for a sleek dark theme, reducing eye strain during long focus sessions.',
            help_m5_h2: 'Full Backups',
            help_m5_l2: 'Exports generate a comprehensive JSON including tasks, labels, language settings, and theme preferences.',
